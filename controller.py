...
def start_app():
    ...
    while True:
        ...
        match choice:
            case 1:
                pb.open_file()
                view.prnt_message(text.load_successful)
            case 2:
                pb.save_file()
                view.prnt_message(text.save_successful)
            case 3:
                view.show_contacts(pb, text.empty_phone_book)
            case 4:
                contact = view.add_contact(text.new_contact)
                pb.new_contact(contact)
                view.prnt_message(text.new_contact_added_successful(contact[0]))
            case 5:
                find_contact(pb)
            case 6:
                find_contact(pb)
            ...
            case 7:
                ...
                view.prnt_message(text.contact_deleted_successful(name))
            case 8:
